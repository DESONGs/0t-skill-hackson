...
def _exec_in_ephemeral_container(args_list):
    ...
    cmd.extend([
        ...
        *(
            ["-e", "AVE_WSS_FIRST_EVENT_ONLY=true"]
            if _env_truthy("AVE_WSS_FIRST_EVENT_ONLY")
            else []
        ),
        "ave-cloud",
        *args_list,
...
def cmd_watch_price(args):
    first_event_only = _env_truthy("AVE_WSS_FIRST_EVENT_ONLY")

    def on_message(ws, message):
        _wss_on_message(ws, message)
        if not first_event_only:
            return
        try:
            body = json.loads(message)
        except json.JSONDecodeError:
        ...
        if isinstance(body, dict):
            ...
            if not event_type and isinstance(body.get("params"), dict):
                event_type = str(body["params"].get("type") or "").lower()
            if not event_type and isinstance(body.get("result"), dict):
                event_type = str(body["result"].get("type") or body["result"].get("topic") or "").lower()
        if event_type == "price":
...
def main():
    ...
    if get_api_plan() == "pro" and not IN_SERVER:
        ...
        if args.command not in _DIRECT:
            if args.command == "watch-price" and _env_truthy("AVE_WSS_DIRECT"):
