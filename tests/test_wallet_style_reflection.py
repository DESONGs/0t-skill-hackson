...
from ot_skill_enterprise.style_distillation.service import (
    _default_wallet_style_display_name,
    _default_wallet_style_slug,
...
class WalletStyleReflectionTests(unittest.TestCase):
    def test_default_wallet_style_name_is_result_focused_and_compact(self) -> None:
        wallet = "0xbac453b9b7f53b35ac906b641925b2f5f2567a89"
        profile = {"primary_archetype": "meme_hunter"}
        self.assertEqual(_default_wallet_style_slug(wallet, "bsc", style_profile=profile), "meme-hunter-bsc-567a89")
        self.assertEqual(_default_wallet_style_display_name(wallet, "bsc", style_profile=profile), "Meme Hunter BSC 567a89")
        self.assertEqual(_default_wallet_style_slug(wallet, "bsc"), "distill-bsc-567a89")
        self.assertEqual(_default_wallet_style_display_name(wallet, "bsc"), "Wallet Distill BSC 567a89")

    def test_explicit_skill_name_stays_predictable_without_hash_suffix(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            reflection = FakeReflectionService(
                ReflectionJobResult(
                    review_backend="pi-reflection-mock",
                    reflection_run_id="run-reflection-custom-name",
                    reflection_session_id="pi-session-custom-name",
                    status="succeeded",
                    raw_output={"text": json.dumps(_mock_minimal_distill_review(wallet, "bsc"))},
                    normalized_output=_mock_minimal_distill_review(wallet, "bsc"),
                    fallback_used=False,
                )
            )
            ...
            try:
                result = service.distill_wallet_style(wallet=wallet, chain="bsc", skill_name="Desk Alpha BSC")
            finally:
                if previous_provider is None:
                    os.environ.pop("AVE_DATA_PROVIDER", None)
                else:
                    os.environ["AVE_DATA_PROVIDER"] = previous_provider
            self.assertEqual(result["candidate"]["target_skill_name"], "Desk Alpha BSC")
            self.assertEqual(result["candidate"]["candidate_slug"], "desk-alpha-bsc")
            self.assertEqual(Path(result["promotion"]["package_root"]).name, "desk-alpha-bsc")
    ...
    def test_wallet_style_distillation_summary_contains_reflection_lineage(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            try:
                result = service.distill_wallet_style(wallet=wallet, chain="bsc")
            finally:
                if previous_provider is None:
                    os.environ.pop("AVE_DATA_PROVIDER", None)
                else:
            ...
            self.assertEqual(result["candidate"]["candidate_slug"], "meme-hunter-bsc-567a89")
            self.assertEqual(result["candidate"]["target_skill_name"], "Meme Hunter BSC 567a89")
            self.assertEqual(Path(result["promotion"]["package_root"]).name, "meme-hunter-bsc-567a89")
