    ...
    *,
    ...
    if style_profile_payload is not None:
        ...
        execute_body = [
            ...
            "        trade_plan['market_context'] = dict(market_context)",
            "        metadata['market_context_injected'] = True",
            "    target_token_context = context.get('target_token_context')",
            "    if isinstance(target_token_context, dict):",
            "        trade_plan['target_token_context'] = dict(target_token_context)",
            "        metadata['market_context_injected'] = True",
            ...
            "        else:",
            "            trade_plan['target_market_snapshot'] = dict(price_context)",
            "        metadata['price_context_injected'] = True",
            "    market_stream_snapshot = context.get('market_stream_snapshot')",
            "    if isinstance(market_stream_snapshot, dict):",
            "        trade_plan['market_stream_snapshot'] = dict(market_stream_snapshot)",
            "        metadata['price_context_injected'] = True",
            "    if metadata.get('market_context_injected') or metadata.get('price_context_injected'):",
            ...
            "        result = execute_skill_action('live', trade_plan, live_intent, project_root=project_root)",
            "    elif mode in {'prepare_only', 'dry_run'}:",
            "        result = execute_skill_action(mode, trade_plan, execution_intent, project_root=project_root)",
            "    else:",
            "        result = {",
            "            'ok': False,",
            "            'mode': mode,",
