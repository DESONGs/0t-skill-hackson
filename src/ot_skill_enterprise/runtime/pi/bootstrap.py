...
def _lockfile_path(runtime_root: Path) -> Path:
    return runtime_root / "package-lock.json"
...
def _install_runtime_dependencies(runtime_root: Path) -> str:
    npm = os.getenv("OT_PI_NPM", "npm")
    lockfile = _lockfile_path(runtime_root)
    if lockfile.is_file():
        _run_command([npm, "ci", "--no-fund", "--no-audit"], cwd=runtime_root)
        return "npm ci"
    _run_command([npm, "install", "--no-fund", "--no-audit"], cwd=runtime_root)
    return "npm install"
...
def _prepare_install_needed(runtime_root: Path) -> bool:
    node_modules = _node_modules_dir(runtime_root)
    if not node_modules.exists():
        return True
    baseline = max(
        _path_mtime(runtime_root / "package.json"),
        _path_mtime(_lockfile_path(runtime_root)),
    )
    return _path_mtime(node_modules) < baseline


def _prepare_build_needed(runtime_root: Path) -> bool:
    artifact = _built_artifact(runtime_root)
    ...
    latest_input = max(
        _path_mtime(runtime_root / "package.json"),
        _path_mtime(_lockfile_path(runtime_root)),
        _directory_mtime(runtime_root / "upstream"),
    )
    return artifact.stat().st_mtime < latest_input
...
def install_pi_runtime_bootstrap(
    *,
    ...
    bootstrap = build_pi_runtime_bootstrap(root=root, workspace_dir=workspace_dir, runtime_root=runtime_root)
    runtime_root_path = bootstrap.runtime_root
    install_mode = _install_runtime_dependencies(runtime_root_path)
    payload = bootstrap.to_dict()
    payload.update(
        {
            "stage": "install",
...
def prepare_pi_runtime_bootstrap(
    *,
    root: Path | None = None,
    workspace_dir: Path | None = None,
    runtime_root: Path | None = None,
    ...
    install_needed = _prepare_install_needed(runtime_root_path)
    build_needed = _prepare_build_needed(runtime_root_path)

    install_mode = None
    if install_needed:
        install_mode = _install_runtime_dependencies(runtime_root_path)

    artifact = bootstrap.built_artifact
    if build_needed:
        artifact = _build_bundle(runtime_root_path)
    ...
    payload.update(
        {
            ...
            "status": "completed",
            "node_modules_present": _node_modules_dir(runtime_root_path).exists(),
            "built_artifact_present": artifact.is_file(),
            "built_artifact_size": artifact.stat().st_size if artifact.is_file() else 0,
            "install": {
                "performed": install_needed,
                "mode": install_mode,
            },
            "build": {
                "performed": build_needed,
            },
            "verify": {
                "performed": verified,
            },
...
def main(argv: list[str] | None = None) -> int:
    ...
    prepare.add_argument("--skip-verify", action="store_true")
    ...
    if args.command in (None, "inspect"):
        ...
        payload = prepare_pi_runtime_bootstrap(
